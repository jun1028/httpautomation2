...
class HttpApiFixture(ExcelColumnFixture):
    ...
    def test(self):
        ...
        try:
            ...
            try:
                ...
                Log.error('saveRespDataToFile error')
    ...
    def doRequest(self, url, reqargs, requestMethod):
        ...
        try:
            ...
            if isinstance(resp, str) or isinstance(resp, unicode) or isinstance(resp, tuple):
                ...
                respData = resp.read()
            Log.debug('respData is ', respData.decode('utf-8').encode('utf-8'))
        except HTTPError, e:
    ...
    def processHeads(self, rowpos, ncols):
        ...
        self.publicParams, rowpos = self.getPublicparams(rowpos, ncols)
    ...
    # @return: interface, rowpos
    def getArgCounts(self, rowpos, ncols=2):
        argCounts = 0
        for col in range(0, ncols):
    ...
    def getPublicparams(self, rowpos, ncols):
        publicParams = {}
        for col in range(0, 2):
            try:
                temp = self.excelAPP.getCellStrValue(rowpos, col)
                if len(temp) > 1 and temp.lower().find('publicparam') > -1:
                    publicParamStr = self.excelAPP.getCellStrValue(rowpos, col + 1)
    ...
    # @return: note, rowpos
    def getComment(self, rowpos, ncols=2):
        note = ''
    ...
    def getJsonpathValueFromResp(self, jsonpathExprStr, resp):
        jsonpathValue = None
        if isinstance(jsonpathExprStr, str) or isinstance(jsonpathExprStr, unicode):
    ...
    def getResultTestCaseFixture(self, execResult):
        Log.debug('start getResultTestCaseFixture: ' + self._CLASSNAME)
        testResult = None
        if execResult and 'actualResult' in execResult :
            if execResult['actualResult']:
                ...
                self.userdefinefixtureresult['actualResult'] = \
                                                    testResult
        Log.debug('end getResultTestCaseFixture: ' + self._CLASSNAME)
        return testResult
...
#        print self.reqargs, type(self.reqargs)
        ...
        if self.reqargs:
            ...
            for reqarg in self.reqargs:
                ...
                if (isinstance(value, str) or isinstance(value, unicode) ) \
                                        and value.find('%') > -1: #如果包含动态变量
                    self.reqargs[reqarg] = \
                        self.getDynamicParamVlaue(value)
        else:
    ...
    def getDynamicParamVlaue(self, paramvalue, fromWhDict={}):
        ...
        try:
            ...
            if not result: #如果当前值没有，则试着从公有变量中获取
                result = self.getKeyValueFromDict(key, self.publiEParams)
        except BaseException, e:
        ...
        if hasattr(self, arg):
            ...
            try:
                if (isinstance(temp, str) or isinstance(temp, unicode) ) \
                    and temp.find('%') > -1: #如果包含动态变量
                    temp = self.getDynamicParamVlaue(temp, self.publicParams)
            except BaseException, e:
                Log.error('arg,', arg)
                Log.error('_addValueToParam error, ', e)
            if temp != 'empty':
        ...
        try:
            for i in range(0, len(results)):
                ...
                if results[i][0] < 0:
                    ...
                    message += 'the ' + str(i)+ ' fail, check message:' \
                    + results[i][1] + '\n'
                else:
                    message += 'the ' + str(i)+ ', pass, check message:' \
                    + results[i][1] + '\n'
        except BaseException, e:
        ...
        if respDict:
            if not key: #如果没有指定key
                for publicparam in self.publicParams:
                    temp = self.getKeyValueFromDict(publicparam, respDict)
                    if temp:
                        self.publicParams[publicparam] = \
                            self.fliterParamValue(publicparam, temp)
            else:
                temp = self.getKeyValueFromDict(publicparam, respDict)
                self.publicParams[publicparam] = self.fliterParamValue(publicparam, temp)
